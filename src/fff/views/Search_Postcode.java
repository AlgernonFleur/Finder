...
public class Search_Postcode extends _View_ {
	...
	@FXML public void initialize(){
		...
		searchBox.setItems(getCurrentPostcodes());
	...
	private void search(){
		ObservableList<Restaurant> results =
			Search.getSearchResultsPostcode(_Overview_.getDatabase().getRestaurants(), searchBox.getValue());
		if(results.size()==0){
	...
	private ObservableList<String> getCurrentPostcodes(){
		ObservableList<String> postcodes = FXCollections.observableArrayList();
		for(Restaurant r:_Overview_.getDatabase().getRestaurants())
			if(!postcodes.contains(String.valueOf(r.getPostcode())))
				postcodes.add(String.valueOf(r.getPostcode()));
